        ...
        <div class="col-md-3">
            ...
            <div class="comments p-2 m-2" style="background-color: rgb(232, 251, 246)">
                @foreach ($product->feedbacks as $feedback)
                    <h5 style=" display: inline-block;">Title: {{$feedback->title}}</h5>
                    <p>&nbsp;{{ $feedback->discription }}</p>
                    <button onclick="get()" id="like" class="btn btn-success">Vote Up({{$feedback->vote_up}})</button>
                    <button onclick="notgood()" class="btn btn-danger">Vote Down({{$feedback->vote_down}})</button>
                @endforeach
            </div>
             ...
             <div class="container m-2 p-2">
                ...
                <form action="" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="comment" id="comment" rows="6" cols="80" placeholder="Enter Comment"></textarea>
                    </div>
            ...
            <div class="comments p-2 m-2" style="background-color: rgb(232, 251, 246)">
                ...
                <h5 style=" display: inline-block;">{{$comment->users->name}}</h5><p style=" display: inline-block; color:blue;">&nbsp;&nbsp;&nbsp;{{$product->created_at->diffForHumans()}}</p>
                    <p style=" display: inline-block;">{{ $comment->comment }}</p>   
                    <form style="display: inline-block;  " action="{{ route('comment.delete',$comment->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button style="display: inline-block; color:red;" type="submit" >Delete</button>
                    </form>
                    <hr>
                @endforeach
            </div>
